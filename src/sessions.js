...
const setupSession = (sessionId) => {
  try {
    ...
    EmitToClient(sessionId, 'start', 'Session initiated successfully')
...
const initializeEvents = (client, sessionId) => {
  ...
  checkIfEventisEnabled('auth_failure')
    .then(_ => {
      client.on('auth_failure', (msg) => {
        EmitToClient(sessionId, 'auth_failure', msg)
  ...
  checkIfEventisEnabled('authenticated')
    .then(_ => {
      client.on('authenticated', () => {
        EmitToClient(sessionId, 'authenticated', "authenticated")
  ...
  checkIfEventisEnabled('call')
    .then(_ => {
      client.on('call', async (call) => {
        EmitToClient(sessionId, 'call', call)
  ...
  checkIfEventisEnabled('change_state')
    .then(_ => {
      client.on('change_state', state => {
        EmitToClient(sessionId, 'change_state', state)
  ...
  checkIfEventisEnabled('disconnected')
    .then(_ => {
      client.on('disconnected', (reason) => {
        EmitToClient(sessionId, 'disconnected', reason)
  ...
  checkIfEventisEnabled('group_join')
    .then(_ => {
      client.on('group_join', (notification) => {
        EmitToClient(sessionId, 'group_join', notification)
  ...
  checkIfEventisEnabled('group_leave')
    .then(_ => {
      client.on('group_leave', (notification) => {
        EmitToClient(sessionId, 'group_leave', notification)
  ...
  checkIfEventisEnabled('group_update')
    .then(_ => {
      client.on('group_update', (notification) => {
        EmitToClient(sessionId, 'group_update', notification)
  ...
  checkIfEventisEnabled('ready')
    .then(_ => {
      client.on('ready', () => {
        EmitToClient(sessionId, 'ready', "Ready")
  ...
  checkIfEventisEnabled('contact_changed')
    .then(_ => {
      client.on('contact_changed', async (message, oldId, newId, isContact) => {
        EmitToClient(sessionId, 'contact_changed', { message, oldId, newId, isContact })
  ...
  checkIfEventisEnabled('chat_removed')
    .then(_ => {
      client.on('chat_removed', async (chat) => {
        EmitToClient(sessionId, 'chat_removed', { chat })
  ...
  checkIfEventisEnabled('unread_count')
    .then(_ => {
      client.on('unread_count', async (chat) => {
        EmitToClient(sessionId, 'unread_count', { chat })
